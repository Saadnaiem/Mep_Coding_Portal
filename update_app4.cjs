...
const startIdx = lines.findIndex(l => l.includes('// --- NOTIFICATION LOGIC ---'));
let endIdx = -1;
for (let i = startIdx + 1; i < lines.length; i++) {
    if (lines[i].includes("setView('dashboard');")) {
        endIdx = i + 1;
        break;
    }
}
...
if (startIdx !== -1 && endIdx !== -1) {
    const replacement = [
        ...
        '        }',
        '        ',
        '        // Notify vendor about completion',
    ...
    console.log('App.tsx replaced successfully without template strings!');
} else {
    console.log('Index mismatch or not found.');
}
