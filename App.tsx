...
const App: React.FC = () => {
  ...
  const employeeInbox = useMemo(() => {
    ...
    return requests.filter(r => {
      ...
      const divisionMatch = myDivisions.length > 0 
        ? myDivisions.some(d => d.toLowerCase() === r.category.toLowerCase())
        : true; // If no assignments found or not a CM, allow (or block depending on strictness)
