...
export const NewVendorForm: React.FC<NewVendorFormProps> = ({ currentVendor, onChange }) => {
  ...
  // FIXED: Removed the aggressive sync effect that was overwriting user input due to debounce lag.
  // Since this is a dedicated form for a new/edit vendor in a wizard, we trust local state while editing.
  // We only initialize from props.
  
  useEffect(() => {
      // Only sync if localForm is empty but currentVendor has data (first load)
      if (Object.keys(localForm).length === 0 && currentVendor && Object.keys(currentVendor).length > 0) {
          setLocalForm(currentVendor);
      }
  }, []); // Run once on mount (or we could depend on currentVendor but check emptiness)
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 zoom-in-95 duration-500">
        <Card title="New Vendor Registration Details" className="border-t-4 border-t-[#0F3D3E]">
             <div className="space-y-6">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <Input label="Contact Person Name" value={getValue('contact_person_name')} onChange={e => handleChange('contact_person_name', e.target.value)} />
                    <Input label="Email Address" type="email" value={getValue('email_address')} onChange={e => handleChange('email_address', e.target.value)} />
                    <Input label="Mobile Number" value={getValue('mobile_number')} onChange={e => handleChange('mobile_number', e.target.value)} />
